...
- **Custom mode** — *free edit, no auto-cost.* Unlocks direct editing (read-only in play mode, to avoid misclicks) of: characteristic **value** (`base`) and **bonus** (`unnatural`, for Unnatural traits), **skills** (rank dropdown: Untrained −20 / Known +0 / Trained +10 / Experienced +20 / Veteran +30), **Spent XP**, **max wounds**, **fatigue max** (override), **fate max**, and the **initiative characteristic** (a dropdown — which characteristic's bonus drives initiative; some talents change it). Dropped talents/powers are not charged. For building characters or reconciling by hand.
...
- **Initiative** — the default initiative bonus is **Agility bonus**, but certain talents change the governing characteristic; make it a **dropdown** (editable **only in Custom advancement mode**), and **wire initiative into the Foundry combat tracker** (roll `1d10 + chosen-characteristic bonus` for the tracker). Currently the Combat sub-tab just *displays* `1d10 + Ag`.
- **Fatigue wiring** — (a) quick **+ / −** buttons beside the Fatigue bar that nudge *current* fatigue by 1 (happens often in play; must NOT require advancement mode); (b) **default max = TB + WB** (RAW), **overridable in Custom advancement** (traits/talents may change it — rare); (c) the **fatigue-affects-characteristics** mechanism (a characteristic whose bonus < current fatigue is halved) — a later focused pass (it cascades through every derived value).
